...
## LimeZu licensed art provider (evaluation / visual-spike only).
##
## LimeZu (Modern Farm/UI/Exteriors/Interiors/Office, Fungus Cave, RPG-Arsenal) is
## being evaluated as a possible future main visual ecosystem. This registry resolves
...
## logical id and kept fully separate so the live Sprout game is untouched.
##
## All LimeZu media stays local-only under gitignored licensed_assets/limezu/. A clean
## checkout with no LimeZu pack must NOT crash: every lookup fails safe to the missing
## placeholder, and the spike shows a clear missing-assets message. Generate/refresh the
## local manifest + candidates with: python tools/art/limezu_integrate.py --all
