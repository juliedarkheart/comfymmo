...
func _ready() -> void:
	...
	_add_button("Give Starter Materials", func() -> void:
		...
		_refresh())
	_add_button("Toggle Plot Debug Overlay", func() -> void:
		if _controller != null and _controller.has_method("admin_toggle_plot_debug"):
			_controller.call("admin_toggle_plot_debug"))
	var teleport_row: HBoxContainer = HBoxContainer.new()
	teleport_row.add_theme_constant_override("separation", 6)
	_rows.add_child(teleport_row)
	for dest in ["landing", "neighborhood", "town"]:
		var tp: Button = Button.new()
		tp.text = String(dest).capitalize()
		tp.pressed.connect(func() -> void:
			if _controller != null and _controller.has_method("admin_teleport"):
				_controller.call("admin_teleport", dest))
		teleport_row.add_child(tp)
