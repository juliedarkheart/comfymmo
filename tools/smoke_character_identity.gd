...
func _initialize() -> void:
	...
	ok = _expect(CharacterAppearance.normalized(default_look) == default_look, "default appearance normalizes to itself") and ok
	...
	custom_look["hair_style"] = "leafy_pigtails"
	...
	ok = _expect(String(restored.get("outfit_color", "")) == "berry_red" \
		and String(restored.get("hair_style", "")) == "leafy_pigtails", "customization saves + restores") and ok
